...
[www.idecon.it](http://www.idecon.it)
...
*   **WEIGHT** - individual weighing, see [WEIGHT](#341-individual-weighing-weight) section.
*   **EVENT** - event or error, see [EVENT](#311-event) section.
*   **STATP** - production statistics, see [STATP](#321-statp) section.
*   **STATPATB** - production statistics, see [STATPATB](#322-statpatb) section.
*   **EndOfBatch** - Batch end statistics, see [EndOfBatch batch closing notification](#352-endofbatch-batch-closing-notification) section.
...
Remember that it is not possible to start the checkweigher with such command in local mode. To know if the checkweigher works in local mode, refer to [Detailed machine status table](#detailed-machine-status-table).
...
Every value represents a weigher status; the meaning of the position is described in [Detailed machine status table](#detailed-machine-status-table).
...
```
It is recommended to verify always if the batch has been opened through the **STATSV** command (refer to the [Detailed machine status table](#detailed-machine-status-table)). Remember that, in 12" devices, if the user tries to open a batch with a previously open batch, an error message will be visualized.
...
BATCHSTOP
```
It is recommended to verify always if the batch has been closed through the **STATSV** command (refer to the [Detailed machine status table](#detailed-machine-status-table)). Remember that, in 12" devices, if the user tries to close a batch with the already closed batch, an error message will be visualized.
...
To interpret the data, refer to [Information of the BATCHINFO batch](#351-information-of-the-batchinfo-batch) section.
...
The filter is an integer value interpreted with a bit mask; the values that can be used are described in the [Mask for filter of the messages sent by the weigher](#mask-for-filter-of-the-messages-sent-by-the-weigher) table.
...
The command without parameters requests the current date and time from the checkweigher; for the response see [DATETIME](#361-actual-date-and-time-datetime) section.
...
```
For additional information, see [DATETIME](#361-actual-date-and-time-datetime) section.
```
...
```
For additional information regarding the value type, refer to the [Table of the types of values](#table-of-the-types-of-values) table.
...
```
In the example above, it can be seen that the **EJECTOR_1** identifier envisions two values, one `INT` type and one `DEC` type. Refer to the [Table of the types of values](#table-of-the-types-of-values) for the explanation of the types.
...
**Supported on devices:** 12"
Parameter identifier to consult/modify the weight category limits type (see [Table of the type of limit of the weight categories](#table-of-the-type-of-limit-of-the-weight-categories) to visualize allowed values).
...
**Supported on devices:** 12"
Parameter identifier to consult/modify the type of debouncing filter. Refer to [Debouncing filters table](#debouncing-filters-table).
...
**Values allowed:** `LOW`, `MEDIUM`, `HIGH`
...
**Supported on devices:** 12"
Parameter identifier to consult/modify the value which represents the action type that must be actuated at the pieces counter expiration. See [Table of executable actions when the counter pieces limit is reached](#table-of-executable-actions-when-the-counter-pieces-limit-is-reached).
...
information]
```
For the list of recipe parameter identifiers see [Recipe Parameter Identifiers](#226-recipe-parameter-identifiers) section.
For additional information regarding the types value refer to the [Table of the types of values](#table-of-the-types-of-values) table.
...
This command always replies with the **PIECE_STAT** message (see [PIECE_STAT](#229-piece_stat) section). In case of insufficient or wrong parameters, the average computed value corresponds to the nominal weight.
...
**Response syntax:**
See [PIECE_STAT](#229-piece_stat) section.
...
**Supported on devices:** 12", 7"
**ATTENTION:** The command described below is outdated, kept in the manual for previous compatibility. We advise against the command usage in favour of **STATSV** command.
Requires the weigher status, for the return codes see [Machine status table](#machine-status-table).
...
**Supported on devices:** 12", 7"
**ATTENTION:** The command described below is outdated, kept in the manual for previous compatibility. We advise against the command usage in favour of single commands used to handle batch and recipe configuration. See Chapter 0 to see an example of production change.
...
The **EVENT** message contains the following information: date, time, production order, batch code, recipe name, line code, checkweigher serial number, event code (see [Event codes table](#event-codes-table)), event description, name and surname of the authenticated user (not managed in all software versions). Each information is separated by the `|` character (ASCII `0x7C`).
...
| %1x   | Classification of the weight (see [Weights classification table](#weights-classification-table)), the value can be a combination of several bit |
...
**Supported on devices:** 12", 7"
The **BATCHINFO** message is sent following a new **BATCHINFO** request, see [BATCHINFO](#220-batchinfo) section.
...
```
In the [EndOfBatch message fields table](#endofbatch-message-fields-table) that follows, the **Overall or batch** column indicates whether the value refers to the overall batch (or production) or to the current split batch.
...
*   [Detailed Machine Status Table](#detailed-machine-status-table)
*   [Batch Data Identifiers Table](#batch-data-identifiers-table)
*   [BATCHINFO Parameters Table](#batchinfo-parameters-table)
*   [Mask for Filter of the Messages Sent by the Weigher](#mask-for-filter-of-the-messages-sent-by-the-weigher)
*   [Table of DATETIME Parameters](#table-of-datetime-parameters)
*   [Table of Examples of Formatting of Numerical Values](#table-of-examples-of-formatting-of-numerical-values)
*   [Table of the Types of ALTERRECIPE Command Errors](#table-of-the-types-of-alterrecipe-command-errors)
*   [Table of the Types of Values](#table-of-the-types-of-values)
*   [Table of the Type of Limit of the Weight Categories](#table-of-the-type-of-limit-of-the-weight-categories)
*   [Debouncing Filters Table](#debouncing-filters-table)
*   [Table of Executable Actions When the Counter Pieces Limit is Reached](#table-of-executable-actions-when-the-counter-pieces-limit-is-reached)
*   [Machine Status Table](#machine-status-table)
*   [Commands Parameters Formatting Conventions Table](#commands-parameters-formatting-conventions-table)
*   [Event Codes Table](#event-codes-table)
*   [STATP Notification Parameters Table](#statp-notification-parameters-table)
*   [STATPATB Notification Parameters Table](#statpatb-notification-parameters-table)
*   [INFORECIPE Notification Parameters Table](#inforecipe-notification-parameters-table)
*   [WEIGHT Notification Parameters Table](#weight-notification-parameters-table)
*   [Weights Classification Table](#weights-classification-table)
*   [EndOfBatch Message Fields Table](#endofbatch-message-fields-table)

## 6. Index of the Commands and Notifications

### A
*   [ALTERRECIPE](#225-modification-of-the-recipe-with-alterrecipe)

### B
*   [BATCHCHANGE](#231-batchchange)
*   [BATCHINFO](#220-batchinfo)
*   [BATCHMODIFY](#219-batchmodify)
*   [BATCHSTART](#26-batchstart)
*   [BATCHSTOP](#27-batchstop)

### D
*   [DATETIME](#224-datetime)
*   [DEBOUNCING_FILTER](#2268-debouncing_filter)
*   [DELETE](#22644-delete)
*   [DISABLESTATS](#213-disablestats)

### E
*   [EJECTOR_1, EJECTOR_2, EJECTOR_3, EJECTOR_4](#2269-ejector_1-ejector_2-ejector_3-ejector_4)
*   [ENABLESTARTBUTTON](#222-enablestartbutton)
*   [ENABLESTATS](#211-enablestats)
*   [EndOfBatch](#352-endofbatch-batch-closing-notification)
*   [ERRNUM](#25-errnum)
*   [EVENT](#311-event)
*   [EXISTS](#22642-exists)

### F
*   [FBK_ENABLE](#22612-fbk_enable)
*   [FBK_K_FACTOR](#22617-fbk_k_factor)
*   [FBK_MAXIMUM_CORRECTION](#22618-fbk_maximum_correction)
*   [FBK_NO_CORRECTION_THRESHOLD](#22616-fbk_no_correction_threshold)
*   [FBK_NON_OPERATION_THRESHOLD](#22621-fbk_non_operation_threshold)
*   [FBK_PIECES_TO_AVERAGE](#22614-fbk_pieces_to_average)
*   [FBK_PIECES_TO_WAIT](#22615-fbk_pieces_to_wait)
*   [FBK_STANDBY_PIECES](#22620-fbk_standby_pieces)
*   [FBK_STANDBY_TIME](#22619-fbk_standby_time)
*   [FBK_TARGET_WEIGHT_ADJUSTER](#22613-fbk_target_weight_adjuster)

### G
*   [GET_CURRENT_PIECE_STAT](#228-get_current_piece_stat)
*   [GETFROMRECIPE](#227-consult-a-recipe-with-getfromrecipe)
*   [GETRECIPELIST](#218-getrecipelist)

### I
*   [INFORECIPE](#217-inforecipe)

### L
*   [LENGTH](#2267-length)
*   [LIMIT_TYPE](#2265-limit_type)
*   [LIMITS_VALUE](#2264-limits_value)
*   [LINECODE](#29-linecode)

### M
*   [MAXIMUM_CONSECUTIVE_EJECTION](#22610-maximum_consecutive_ejection)
*   [METAL_ANALISYS_MODE](#22629-metal_analisys_mode)
*   [METAL_FREQUENCY](#22628-metal_frequency)
*   [METAL_PHASE](#22627-metal_phase)
*   [METAL_REVELATION_MODE](#22630-metal_revelation_mode)
*   [METAL_SENSITIVITY](#22626-metal_sensitivity)
*   [METAL_TESTER_DIAMETERS](#22632-metal_tester_diameters)
*   [METAL_Z](#22631-metal_z)
*   [MOTOR_4_SPEED_PERCENTAGE_COMPARED_MOTOR_2](#22624-motor_4_speed_percentage_compared_motor_2)
*   [MOTOR_STOP_DELAY_ON_EXIT_PTC](#22636-motor_stop_delay_on_exit_ptc)
*   [MSGFILTER](#221-msgfilter)

### N
*   [NEW](#22643-new)

### O
*   [OPTION_A](#22622-option_a)
*   [OPTION_B](#22623-option_b)

### P
*   [PERCENTAGE_OF_PRODUCT_M_TO_ACCEPT](#22611-percentage_of_product_m_to_accept)
*   [PIECE_STAT](#229-piece_stat)
*   [PRODUCT_CODE](#2263-product_code)

### R
*   [RECIPE](#23-recipe)
*   [RESETERRORI](#210-reseterrori)

### S
*   [SCREWFEEDER_PTC_IN_OUT_TIMEOUT](#22625-screwfeeder_ptc_in_out_timeout)
*   [SELSTATSANSWER](#214-selstatsanswer)
*   [SHOWMESSAGE](#223-showmessage)
*   [SHUTDOWN](#28-shutdown)
*   [SPEED_CONVEYOR_BELT_1](#22633-speed_conveyor_belt_1)
*   [SPEED_CONVEYOR_BELT_2](#22634-speed_conveyor_belt_2)
*   [SPEED_CONVEYOR_BELT_3](#22635-speed_conveyor_belt_3)
*   [START](#21-start)
*   [STATCADENCY](#212-statcadency)
*   [STATP](#321-statp)
*   [STATPATB](#322-statpatb)
*   [STATREQ](#215-statreq)
*   [STATREQATB](#216-statreqatb)
*   [STATSV](#24-statsv)
*   [STATUS](#230-status)
*   [STOP](#22-stop)
...
### T
*   [TARE](#2261-tare)
*   [THROUGHPUT](#2266-throughput)

### U
*   [USRCNT_ACTION_RESET](#22639-uscnt_action_reset)
*   [USRCNT_ON_EXPIRATION_ACTION](#22638-uscnt_on_expiration_action)
*   [USRCNT_OUTPUT_ACTIVATION_TIME](#22640-uscnt_output_activation_time)
*   [USRCNT_RESET_VALUE](#22637-uscnt_reset_value)

### W
*   [WEIGHT](#341-individual-weighing-weight)
